...
import gsap from "https://cdn.jsdelivr.net/npm/gsap@3.12.3/+esm";
...
const vertex = `
          ...
          void main() {
            vUv = resizeUvCover();
            gl_Position = vec4(position, 0, 0.9);
          }
`;
  ...
  class BulgeImage {
    ...
    resize = () => {
      ...
      if (this.#program) {
        ...
        const image = new Image();
        const textureGl = new Texture(this.#renderer.gl);

        image.onload = () => {
          textureGl.image = image;
          this.#program.uniforms.uTexture.value = textureGl;
        };
